...
| Q2-I-005 | 5 m 批量 strict classification 超时 | sparse generation 已降至0.512 s且M4禁用时整体仍超300 s，定位到 `StrictCertificateAccelerator.certify_points` 的重复 anchor 距离传播 | C | OPEN | 实现严格等价 incremental anchor propagation 并只重跑 region-only | Q2-robust-selection-v2.1 | 不采用KDTree/并行/近似；物理阈值和证书判据不变 |
