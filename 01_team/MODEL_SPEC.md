...
- Mathematical Model：`P1_bound=D∩W(S1,theta1_hat,1 degree)∩B(S1,1500)`；`P1_phys={G∈P1_bound: ||G||<=1800, 5<||G-S1||<=1500}`；`receive_floor(G)=max(1000,||G-S1||)` 仅表示与第一次成功接收相容的最小可能 `rho`，不是实际物理接收半径。严格鲁棒接收当且仅当对所有 `G∈P1_phys` 均有 `||G-S2||<=receive_floor(G)`（数值判据 `Vrec=max_G[||G-S2||-receive_floor(G)]<=eps_rec`）。证书分为 `CERTIFIED_STRICT`、`CERTIFIED_VIOLATION`、`UNRESOLVED`；只有前者令 `strict_receive=True` 并进入 `Cstrict`，后两者分别为 `False` 和 `None`。禁止以 `Q==1` 定义严格集。
...
- Search and Selection：`Omega_move` 默认 `None`；无额外边界时搜索盒为 `bbox(P1_bound)` 四周各扩 1500 m。`Cstrict={certificate.status==CERTIFIED_STRICT and ||S2-S1||>=Lmin}`，默认 `Lmin=50 m`。`CERTIFIED_VIOLATION` 排除；`UNRESOLVED` 必须继续细分、增加预算或显式报告，禁止静默作为 infeasible 丢弃。若 `C20` 非空按 `(T2,JR,JD,x,y)` 字典序选择；否则若 `Cstrict` 非空，先对 `(JR,JD,T2)` 作 tolerance-aware Pareto，再按 `(JR,JD,T2,x,y)` 选择；严格集为空时正式推荐为 `None`，风险点不得升级。
